...
\section{Hardware}
%%Add picture of cadmodel testrig?!
...
calculates what current should be applied to the motor (explained further
in Section~\ref{sec:cardynamics} and~\ref{sec:rigdynamics}), this reference signal
...
Using the London track and the plant model of Elba, $P_m$ during one lap could
be simulated, as shown in (\ref{fig:testrig_power_required}).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./img/testrig_power_required.eps}
    \caption{Power required for the motor on the
    testrig.}\label{fig:testrig_power_required}
\end{figure}

Figure~\ref{fig:testrig_power_required} shows that the maximum required power
...
To acquire the motor parameters, calculations had to be made, since it did not exist any datasheet to the DC motor in use. The parameters needed was the internal resistance, $R_{a}$, and the motor constant, $k_{\phi}$. The motor was run at idle for two different voltage levels and the current and the speed in terms of RPM (revolutions per minutes) where noted for those specific voltage levels. The speed in angle velocity was calculated from RMP, and together with the current and voltage for two different instances, $R_{a}$ and $k_{\phi}$ was calculated with 
\begin{equation}
\label{eq:motor_parameters}
    U = R_a I + k_{\phi} \omega
\end{equation}
found in \cite{elektroteknik2013} put  an equation system. 
